...
inline void EvalQuantizedPerChannel(TfLiteContext* context, TfLiteNode* node,
  ...
  if (dilation_width_factor == 1 && dilation_height_factor == 1) {
    ...
    data_dims_t input_dims = {.channels = input_depth};

    data_dims_t output_dims = {.channels = output_depth};
...
#if ESP_NN
  if (input->type == kTfLiteInt8) {
    data_dims_t input_dims = {.channels = input->dims->data[3]};

    data_dims_t output_dims = {.channels = output->dims->data[3]};
