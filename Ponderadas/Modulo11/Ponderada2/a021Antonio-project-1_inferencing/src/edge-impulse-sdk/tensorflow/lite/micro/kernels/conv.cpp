...
#if ESP_NN
  if (input->type == kTfLiteInt8) {
// Linha corrigida para input_dims
    data_dims_t input_dims = {.width = input_width, .height = input_height, .channels = input->dims->data[3]};

// Linha corrigida para output_dims
    data_dims_t output_dims = {.width = output_width, .height = output_height, .channels = output->dims->data[3]};

// Linha corrigida para filter_dims (removendo .depth e .batch)
    data_dims_t filter_dims = {.width = filter_width, .height = filter_height, .channels = 0};
...
inline void EvalQuantizedPerChannel(
  ...
  if (dilation_width_factor == 1 && dilation_height_factor == 1) {
    ...
    // Linha corrigida para input_quantized_dims
    data_dims_t input_quantized_dims = {.width = input_width, .height = input_height, .channels = input_depth};

    // Linha corrigida para output_quantized_dims
    data_dims_t output_quantized_dims = {.width = output_width, .height = output_height, .channels = output_depth};

    // Linha corrigida para filter_quantized_dims (removendo .depth e .batch)
    data_dims_t filter_quantized_dims = {.width = filter_width, .height = filter_height, .channels = 0};
    ...
    for (int i_batch = 0; i_batch < batch_size; i_batch++) {
      esp_nn_conv_s8(&input_quantized_dims, input_data + i_batch * input_size,
                     &filter_quantized_dims, tflite::micro::GetTensorData<int8_t>(filter),
                     tflite::micro::GetTensorData<int32_t>(bias),
                     &output_quantized_dims, output_data + i_batch * output_size,
